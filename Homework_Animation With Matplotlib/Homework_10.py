...
div_row3 = html.Div(children =	[div_3_graph],
...
nmice_timepoint=pd.DataFrame(merged_df, columns = ["Mouse ID",'Timepoint','Drug Regimen', 'Tumor Volume (mm3)'])
nmice_timepoint = nmice_timepoint.groupby(['Timepoint', 'Drug Regimen']).count().reset_index()
...
def update_survival_function (drug_list):
    ...
    if drug_list:
	    for drug_name in drug_list:
		    traces.append(go.Scatter(x=nmice_timepoint[nmice_timepoint['Drug Regimen']==drug_name]['Timepoint'],
		    	                     y=nmice_timepoint[nmice_timepoint['Drug Regimen']==drug_name]['Mouse ID'],
		    							opacity=0.6,
		    							name = drug_name,
		    							marker = dict(color=drug_colors[drug_name]),
		    			                mode = 'lines+markers'
    ...
    return {
        ...
        'layout': dict(
            ...
            xaxis={'title': 'Timepoint',
   					'range': [nmice_timepoint['Timepoint'].min(), nmice_timepoint['Timepoint'].max()]
   					},
            yaxis={'title': 'number of mice', 
            		'showgrid': False,
            		'showticklabels': False
...
metastat_timepoint =  merged_df.groupby(['Drug Regimen','Timepoint']).mean()[['Metastatic Sites']]
metastat_timepoint= metastat_timepoint.reset_index()
...
def update_metastatic_graph(drug_list):
    traces = []
    if drug_list:
	    for drug in drug_list:
	    	traces.append(go.Scatter(x = metastat_timepoint[metastat_timepoint['Drug Regimen']==drug].index,
		    	y = metastat_timepoint[metastat_timepoint['Drug Regimen'] == drug]['Metastatic Sites'],
		    							opacity=0.6,
		    							name = drug,
		    							marker = dict(color=drug_colors[drug])
